...
void handle_status() {  
  ...
  json += " \"gpios\":" + readGPIOS() + ",\n";
  json += " \"sensors\":" + readSensors() + ",\n";
  json += "}";
...
void handle_gpios_status() {  
  String json = readGPIOS();
  server.send(200, "application/json", json);
  json = String();
}

void handle_switch_arg(int relayNb) {
...
void handle_gpios_switch() {
  ...
  {
    server.send(400, "text/plain", "Bad Parameter !");  
  }  
}

void handle_teleinfo() {
  unsigned long timeCurrent = millis();
  unsigned long timeEnd = timeCurrent + 10000L;
  Serial.print("/teleinfo\ntimeC=" + String(timeCurrent) + "\ntimeE=" + String(timeEnd));
  
  String frameTeleInfo = "plop";
  char charIn = 0;
  char lookedAt = startFrame;
  while (timeCurrent < timeEnd) {
    ...
    if (teleinfo.available()) {
      ...
      if(lookedAt == endFrame) {
        Serial.print("...got end frame");
        //frameTeleInfo += String(charIn);
      }
  
      if(charIn == startFrame) {
        Serial.print("...got start frame");
        //lookedAt = endFrame;        
      }
    }    
  }
  server.send(200, "text/plain", frameTeleInfo);
}

String readSensors() {
  String json = "{\n";
  json += " \"temperature\": " + String(sensorTH.readTemperature()) + ",\n";
  json += " \"humidity\": " + String(sensorTH.readHumidity()) + ",\n";
  json += "}";
  return json;
}
...
void setup(void){
  ...
  server.on("/teleinfo", HTTP_GET, handle_teleinfo);
